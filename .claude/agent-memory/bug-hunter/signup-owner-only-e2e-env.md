...
- `ENG-001` (Microsoft OAuth signup): now self-skips (`test.skip` inside the test) when Entra creds are absent. `.env` currently has `AUTH_MICROSOFT_ENTRA_ID_ID` set.
- `ENG-002` (logout redirect): times out waiting for `**/dashboard` after logging in as `admin@test.com` (seeded admin password: see `prisma/seed.ts`) — likely a stale/missing seed user for that persona post-RBAC-migration.
...
Related: [[project-test-framework]], [[notification-engine-test-patterns]].

**SMTP as of 2026-09:** the Zoho SMTP lines in `.env` are commented out, and `.env.local` sets `SMTP_HOST=localhost` / `SMTP_PORT=1025`. E2E mail goes to the `lms-e2e-smtp` container under `npm run e2e:local`. This config has flipped before, so check `SMTP_HOST` directly before trusting this note.
