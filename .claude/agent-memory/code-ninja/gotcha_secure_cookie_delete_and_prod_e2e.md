...
- `/onboarding-worker` (`src/app/onboarding-worker/page.tsx`) renders `<Image src="/images/login-bg.png" priority fill>` on a 6.5MB PNG (`quality={100}` has since been dropped, which reduces but has not been proven to remove the risk). When the org-less-worker login redirect (`/worker` → `/onboarding-worker`) aborts the in-flight `/_next/image?...w=1920&q=100` optimization, the page's duplicate request for the same cache key coalesces onto the aborted generation and **hangs forever** (reproduced: a 2nd request for the same uncached image after aborting the 1st returns `http=000` at 30s; curl warm/cold alone = 1.4s). This makes `page.goto(..., {waitUntil:'load'})` time out. Framework-level, unrelated to auth. Remaining remediations: use a smaller optimized asset / don't mark it `priority`, or have the e2e use `waitUntil:'domcontentloaded'`.
