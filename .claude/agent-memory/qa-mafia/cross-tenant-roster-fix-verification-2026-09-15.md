...
**CONFIRMED FIX — Due Date can now genuinely be cleared** (previously it could only be set, never removed — see [[dashboard-scoping-fix-2026-09-14]]'s "Assign-form Due Date has no clear affordance" note, now resolved). A "Clear due date" icon-button appears next to the date once set; clicking it, saving, and reopening shows a real empty state (placeholder text, no leftover time). **Training Schedule got the same "Clear schedule date" affordance too** (unprompted regression-adjacent improvement, not itself the target of this story).
...
**D01 admin's password was reset again this round** (`theraptlyqa+d01admin@gmail.com` → `QaTest123!D01`) via the standard forgot-password + QA Gmail IMAP flow — same recipe as prior rounds, still works cleanly, zero rate-limit friction (first reset request of the session).

See [[staging-qa-fixture-orgs]], [[dashboard-scoping-fix-2026-09-14]], [[assign-surface-consolidation-2026-09-14]], [[course-delete-and-withdraw-mechanics]], [[qa-gmail-imap-quirks-and-ui-bypass-technique]] for related background.
