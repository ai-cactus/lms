...
description: Document Hub upload/PHI-scan/viewer/rename/delete flow, DOCX fixture generation via Node (pdfkit PDFs fail pdf-parse), and the now-fixed document.* RBAC gate on both action and page
metadata:
  type: reference
---

**Generating PDF/DOCX test fixtures when Python tooling is unavailable:** this environment had no `pip`/`pip3`, and Python's `docx`/`reportlab`/`fpdf` modules were not installed, so `python-docx`/`reportlab` cannot be assumed available. **Working fallback:** Node/npm was available — `npm install docx` (in a scratchpad dir, not the repo) produces valid DOCX files via small one-off scripts. **Prefer DOCX:** pdfkit and pdf-lib PDFs are well-formed but fail the app's pdf-parse text extractor (see [[local-dev-env-setup]]). Verify output with `file <path>` (expect "PDF document..." / "Microsoft Word 2007+"). Don't assume `libreoffice`/`soffice`/`pandoc` exist either — check first.
...
**HISTORICAL (2026-07-01, production): the document list refreshed unreliably after upload and gave no PHI-scan feedback.** PHI rejection now shows a real toast; see [[course-creation-flow-redesign-2026-09-12]] for the live PHI-block proof. The bullets below record the original observations:
...
**RBAC gate (2026-07-27, `0bcebc3`, then the page fix):** `uploadDocument`, `getDocuments`, `deleteDocument` and `renameDocument` check `can(roleKey,'document.*')` server-side. The Documents Hub page (`src/app/dashboard/(main)/documents/page.tsx`) is gated the same way: `canCreate = can(roleKey,'document.create')` renders `UploadSection`, and `canDelete` gates delete. Finance holds no `document.*` permission, so it no longer sees "Upload New". The earlier gap, where the page used the coarse `isAdminRole` gate, is FIXED.
