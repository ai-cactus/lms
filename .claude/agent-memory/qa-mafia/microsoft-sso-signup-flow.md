...
**Microsoft's own login UI for a personal account defaults to passwordless "Get a code" (email OTP).** To use a provided password instead: click **"Other ways to sign in"** → **"Use your password"** → enter password → Next. No MFA/OTP challenge appeared for the tested account (`idowuseyi22@gmail.com`), and no OAuth consent/permissions screen appeared either (possibly pre-consented app registration) — only a "Stay signed in?" prompt (click No/Yes, either is fine, doesn't affect app-side outcome for a throwaway test session).
