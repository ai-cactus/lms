...
**CSV import silently drops malformed rows with zero error UI, in BOTH CSV import paths** (onboarding step5 "Import with .csv file" AND Staff Management "Add Workers" → "Click to upload .csv file instead"). Tested with a CSV mixing valid rows + a missing-email row (`,nurse`) + a malformed-email row (`not-an-email,nurse`): only the valid rows import, with just a bare count shown ("N valid emails found" / "N contacts imported") — no toast, banner, or console message ever names or counts the skipped rows. If a QA pass needs to verify "malformed rows flagged with a clear error," this currently FAILS — don't assume it passes just because the valid rows import correctly.
