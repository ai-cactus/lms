...
- `offering.ts:53–99` `listAvailableVideoCourses` — confirmed as the global-catalog candidate for `unstable_cache`. **Important nuance:** the catalog is only *mostly* org-independent — the query does `offerings: { where: { organizationId }}` inline, so the whole result as written is per-org. The cacheable part is the published-global-course + lesson/quiz-count shape; the per-org "is this offered" flag must be joined **after** the cache read, not baked into the cached payload, or the cache key/tag design balloons to one entry per org (defeating the purpose and risking a slow first-hit per org). This needs to be split into (a) a cached `getGlobalVideoCatalog()` and (b) a small per-org offerings lookup merged client-side of the cache boundary.
  - **Superseded 2026-08-28:** the split shipped, and `listAvailableVideoCourses` has since been **deleted** — the Courses-page consolidation dropped the outer "My Courses" / "Available Video Courses" tabs, so nothing rendered it any more. `getGlobalVideoCatalog()` survives and is now consumed by `listGlobalVideoCatalogCourses()`, which joins per-org enrollment tallies (not adoption state) after the cached read. The caching design described here is intact; only the caller's name changed.
...
- **Superseded 2026-08-28:** implemented, then the caller was removed. There is no "available courses" browse route any more — the global catalog is merged into the Video tab of the single Courses list. Re-point the e2e assertion and the measurement gate at `/dashboard/courses` if this gate is ever re-run.
