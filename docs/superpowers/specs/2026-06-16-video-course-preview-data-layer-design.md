...
> **Adoption retired 2026-08-10.** The "shared offering" adoption model (§3 — `offerCourseToOrg` creating an `OrgCourseOffering`) and the org-facing "Choose a Prebuilt Course" flow were removed by the 2026-08-10 video-course ruling; global video courses are assigned directly and `OrgCourseOffering` is no longer populated. The `offerCourseToOrg` / `updateOffering` / `withdrawOffering` server actions were deleted on 2026-09-23 (founder ruling on Q-09); nothing in the app calls them. See BUG-01 (`docs/local/OPEN-ISSUES.md`).
