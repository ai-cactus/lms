-- 1. Fix missing enum value (must be run first)
-- Note: If this fails saying "cannot run inside a transaction block", run this line ALONE first.
...
DROP POLICY IF EXISTS "Admins and Supervisors can view all attempts" ON quiz_attempts;
...
DROP POLICY IF EXISTS "Admins and Supervisors can view all answers" ON quiz_answers;
...
DROP POLICY IF EXISTS "Users can insert their own answers" ON quiz_answers;
