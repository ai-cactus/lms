...
function parseDualOutput(rawResponse: string): { jsonStr: string; markdown: string } {
    ...
    if (markdown.startsWith('```markdown')) {
        markdown = markdown.replace(/^```markdown\s*/, '').replace(/```\s*$/, '').trim();
    }
...
async function generateArticleV46(
    ...
    if (!result.success) {
        ...
        throw new Error(`ArticleMeta validation failed: ${result.error.issues.map(i => i.message).join('; ')}`);
    }
...
async function generateSlidesV46(
    ...
    if (!result.success) {
        ...
        throw new Error(`Slides validation failed: ${result.error.issues.map(i => i.message).join('; ')}`);
    }
...
async function generateQuizV46(
    ...
    const prompt = buildPromptC_v46(articleMarkdown, articleMetaJson, requestedQuestionCount, quizDifficulty);
    ...
    if (!result.success) {
        ...
        throw new Error(`Quiz validation failed: ${result.error.issues.map(i => i.message).join('; ')}`);
    }
...
async function judgeQuizV46(
    ...
    if (!result.success) {
        ...
        throw new Error(`Judge validation failed: ${result.error.issues.map(i => i.message).join('; ')}`);
    }
...
async function regenFlaggedV46(
    ...
    const prompt = buildPromptE_v46(articleMarkdown, articleMetaJson, quizJson, judgeJson, quizDifficulty);
    ...
    if (!result.success) {
        ...
        throw new Error(`Regen validation failed: ${result.error.issues.map(i => i.message).join('; ')}`);
    }
...
export async function generateCourseAndQuizV46(formData: FormData): Promise<{ jobId?: string; error?: string }> {
    ...
    if (file) {
        try {
            ...
            console.error('[v4.6] File parsing error:', err);
        ...
        try {
            ...
            console.log(`[v4.6] Read ${sourceText.length} characters from stored document: ${docFilename}`);
            sourceText = truncateToContext(sourceText, MAX_SOURCE_TOKENS);
        } catch (err: any) {
            console.error('[v4.6] DB document read error:', err);
            return { error: `Failed to read stored document: ${err.message}` };
        }
    } else {
    ...
    processBackgroundV46(job.id, sourceText, docFilename, rawData).catch(err => {
        console.error('[v4.6] Background job failed fatally:', err);
    });
    ...
    try {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                console.error(`[v4.6 Background] Stage A attempt ${attempt} failed:`, error.message);
                if (attempt === maxAttempts) errorMsg = error.message;
        ...
        const [slidesResult, quizResult] = await Promise.allSettled([
            ...
            (async () => {
                for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                    try {
                        console.log(`[v4.6 Background] Stage B attempt ${attempt}/${maxAttempts} for job ${jobId}`);
                        return await generateSlidesV46(articleMarkdown, rawArticleMetaJson, desiredSlideCount);
                    } catch (error: any) {
                        console.error(`[v4.6 Background] Stage B attempt ${attempt} failed:`, error.message);
            ...
            (async () => {
                for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                    try {
                        console.log(`[v4.6 Background] Stage C attempt ${attempt}/${maxAttempts} for job ${jobId}`);
                        return await generateQuizV46(articleMarkdown, rawArticleMetaJson, questionCount, difficulty);
                    } catch (error: any) {
                        console.error(`[v4.6 Background] Stage C attempt ${attempt} failed:`, error.message);
                        if (attempt === maxAttempts) throw error;
                        await new Promise(r => setTimeout(r, 1000 * attempt));
                    }
                }
        ...
        if (quizJson) {
            ...
            try {
                ...
                if (flaggedCount > 0 && MAX_REGEN_CYCLES > 0) {
                    console.log(`[v4.6 Background] Stage E: ${flaggedCount} flagged questions → regenerating for job ${jobId}`);
                    try {
                        ...
                        console.log(`[v4.6 Background] Stage E: patched ${regenQuestions.length} questions for job ${jobId}`);
                    } catch (regenErr: any) {
                        console.error(`[v4.6 Background] Stage E failed (non-fatal):`, regenErr.message);
                ...
                console.error(`[v4.6 Background] Stage D failed (non-fatal):`, judgeErr.message);
                // Non-fatal — keep quiz without judge review
            }
        }

        console.log(`[v4.6 Background] Pipeline complete for job ${jobId}. Sections: ${articleMeta.sections.length}, Slides: ${slidesJson?.slides.length || 0}, Questions: ${quizJson?.questions.length || 0}`);
        ...
        await prisma.job.update({
            where: { id: jobId },
            data: { status: 'completed', result: resultPayload as any },
        });
    } catch (err: any) {
        console.error(`[v4.6 Background] Uncaught fatal error in job ${jobId}:`, err);
        await prisma.job.update({
            where: { id: jobId },
            data: { status: 'failed', payload: { error: err.message || 'Unknown server error during background processing' } },
        });
    }
}

export async function checkCourseGenerationJobV46(jobId: string) {
    try {
        const job = await prisma.job.findUnique({ where: { id: jobId } });
        if (!job) return { error: 'Job not found' };

        if (job.status === 'completed') {
            return { status: 'completed', result: job.result as any };
        ...
        return { error: `Failed to check job: ${err.message}` };
    }
}
