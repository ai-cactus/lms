...
// ---------------------------------------------------------------------------
const { mockAdminAuth, mockWorkerAuth, mockCourseFindMany, mockEnrollmentGroupBy } = vi.hoisted(
  () => {
...
import { listGlobalVideoCatalogCourses } from './offering';
...
/**
 * Catalog gate: tier AND verb.
 *
 * The tier check is composed with the verb rather than replaced by it:
 * `workerPermissions` grants every learner `course.read`, so gating the read on
...
describe('offering permission gates', () => {
  ...
  it('allows a supervisor to READ the catalog — they hold course.read', async () => {
    setSession('supervisor');

    await expect(listGlobalVideoCatalogCourses()).resolves.toBeDefined();
  });

  it('refuses a worker the catalog read — the tier check is what stops them', async () => {
