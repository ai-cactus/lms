...
const session = { user: { id: OWN_USER } };
...
function courseOwnedBy(ownerUserId: string) {
  return {
    id: 'course-1',
...
describe('generateSingleQuestion — access control', () => {
  ...
  it('refuses a course belonging to another organization', async () => {
    prismaMock.course.findUnique.mockResolvedValue(courseOwnedBy(OTHER_USER));
  ...
  it('allows a course owned by the caller organization', async () => {
    prismaMock.course.findUnique.mockResolvedValue(courseOwnedBy(OWN_USER));
