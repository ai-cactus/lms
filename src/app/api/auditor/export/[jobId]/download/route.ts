...
export async function GET(req: NextRequest, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    const result = job.result as unknown as AuditReportResult;
    const flatRows = flattenAuditReport(result);
    ...
    await audit({
      ...
      targetType: 'job',
      targetId: jobId,
      metadata: { scope: result.scope, format, rowCount: flatRows.length },
