    ...
    try {
        ...
        if (enrollment.status === 'locked') {
            return NextResponse.json({ 
                ...
                message: 'You have used all allowed attempts for this quiz. An admin must assign a retake.'
            }, { status: 403 });
        }
        ...
        const message = result.status === 'resumed' ? 'Resumed active attempt' 
            : result.status === 'created' ? 'Started first attempt' 
            : 'Started new attempt';
