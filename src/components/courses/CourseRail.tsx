        ...
        <nav className={`${styles.rail} ${className}`}>
            ...
            {lessons.map((lesson, i) => {
                ...
                return (
                    <button
                        key={i}
                        className={`${styles.modThumb} ${isActive ? styles.modThumbActive : ''} ${isDone ? styles.modThumbDone : ''} ${isLocked ? styles.modThumbLocked : ''}`}
            ...
            {quiz && (
                <button
                    className={`${styles.modThumb} ${styles.modThumbQuiz} ${activeIndex === lessons.length ? styles.modThumbQuizActive : ''} ${lessons.length > unlockedIndex ? styles.modThumbLocked : ''}`}
                    onClick={() => lessons.length <= unlockedIndex && onSelect(lessons.length)}
                    disabled={lessons.length > unlockedIndex}
