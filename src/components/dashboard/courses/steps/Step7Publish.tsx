    ...
    <div className={styles.stepWrapper}>
      ...
      <div style={{ width: '100%', maxWidth: 600 }}>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <div
            className={styles.chipInputWrapper}
            ref={wrapperRef}
          ...
          >
            {(data.assignments || []).map((item: string, index: number) => {
              ...
              return (
                <div
                  key={index}
                  className={`${styles.chip} ${isNewInvite ? styles.chipInvite : ''}`}
                >
                  {item}
            ...
            {showSuggestions && (inputValue.length >= 2 || suggestions.length > 0) && (
              <div className={styles.autocompleteDropdown}>
                {isLoading ? (
                  <div style={{ padding: '10px', color: '#718096', fontSize: '14px' }}>
                    Searching...
                  </div>
          ...
          {validationError && (
            <div style={{ color: '#E53E3E', fontSize: '13px', marginTop: '6px' }}>
              {validationError}
            </div>
          )}
        </div>
