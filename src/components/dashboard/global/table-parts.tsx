...
export const facilityTableHeadClass =
  'h-10 bg-background-secondary px-4 text-xs font-medium whitespace-nowrap text-text-secondary';
...
export function FacilityTableSection({
  ...
  return (
    <section className="flex flex-col gap-5 rounded-[17px] border border-border bg-background p-4 shadow-sm sm:p-5">
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex min-w-0 flex-col">
        ...
        {type && <span className="truncate text-xs text-text-secondary">{type}</span>}
      </div>
    </div>
  );
}
