...
export default function AssignUserCourseModal({ isOpen, onClose, userEmail, userName, enrolledCourseIds, onSuccess }: AssignUserCourseModalProps) {
    const [courses, setCourses] = useState<{ id: string, title: string }[]>([]);
    const [selectedCourseId, setSelectedCourseId] = useState<string>('');
    const [isLoading, setIsLoading] = useState(false);
    const [isFetching, setIsFetching] = useState(false);
    const [result, setResult] = useState<{ success: string[]; alreadyEnrolled: string[]; failed: string[] } | null>(null);
                ...
                <div className={styles.selectWrapper}>
                    ...
                    >
                        <option value="" disabled>
                            {isFetching ? 'Loading courses...' : courses.length === 0 ? 'User has been assigned all available courses' : 'Select a course...'}
                        </option>
                        {courses.map(course => (
