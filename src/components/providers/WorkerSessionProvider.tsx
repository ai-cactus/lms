...
interface WorkerSessionProviderProps extends WithChildren {
  /** Account the server resolved for this request; enables the per-tab guard. */
  currentUserId?: string;
  currentUserName?: string;
}

export const WorkerSessionProvider: FC<WorkerSessionProviderProps> = ({
  children,
  currentUserId,
  currentUserName,
}) => {
  return (
    <SessionProvider basePath="/api/auth-worker" refetchOnWindowFocus>
      <SessionIdentityGuard currentUserId={currentUserId} currentUserName={currentUserName}>
        ...
        <InactivityTimer />
        {/* Inside the provider so useSession() resolves against the WORKER session
            rather than the admin one — the two are separate contexts. Inside the
