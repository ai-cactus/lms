...
export function WizardContainer({ onClose, onComplete, initialPolicyIds, initialDraft, forceNewDraft }: WizardContainerProps) {
    ...
    const generateCourseContent = async () => {
        ...
        try {
            ...
            // Read selected existing policies
            const existingPolicyContents: { name: string; type: string; content: string }[] = [];
            if (selectedPolicyIds.length > 0) {
                const { data: policies } = await supabase
                    .from("policies")
                    .select("*")
                    .in("id", selectedPolicyIds);
                    ...
                    const processedConfigs = await Promise.all(
                        policies.map(async (policy) => {
                            try {
                                ...
                                console.error(`Error processing policy ${policy.file_name}:`, error);
                                return null;
                            }
                        })
                    );

                    processedConfigs.forEach(pc => {
                        if (pc) existingPolicyContents.push(pc);
                    });
                }
            }

            const fileContents = [...newFileContents, ...existingPolicyContents];

            if (fileContents.length === 0) {
                throw new Error("No content available to generate course. Please upload documents or select existing policies.");
            }
    ...
    const handleNext = async () => {
        if (step === 2) {
            // If we have selected existing documents, we need to analyze them now
            if (selectedPolicyIds.length > 0) {
                setIsAnalyzing(true);
                try {
                    const { data: policies } = await supabase
                    ...
                    if (policies && policies.length > 0) {
                        const fetchedFiles = await Promise.all(
                            policies.map(async (p) => {
                                ...
                                return new File([blob], p.file_name, { type: blob.type });
                            })
                        );

                        const success = await performAnalysis(fetchedFiles);
                        if (!success) {
                            setIsAnalyzing(false);
                            return;
                        }
                    ...
                    showNotification("error", "Failed to process selected documents");
                    setIsAnalyzing(false);
                    return;
                }
            }
    ...
    const canProceed = () => {
        ...
        if (step === 2) {
            // Can proceed if:
            // 1. Files uploaded and analyzed (uploadProgress === 100)
            // 2. OR pre-existing documents selected (selectedPolicyIds.length > 0)
            const hasAnalyzedFiles = files.length > 0 && !isAnalyzing && uploadProgress === 100;
            const hasSelectedExistingDocs = selectedPolicyIds.length > 0;
            return hasAnalyzedFiles || hasSelectedExistingDocs;
        }
