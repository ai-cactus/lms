...
describe('collectDeferredNotices', () => {
  it('groups deferred payloads by membership into one notice per worker', () => {
    const notices = collectDeferredNotices([
      deferred({ organizationUserId: 'org-user-1', userId: 'user-1', courseId: 'course-1' }),
      deferred({ organizationUserId: 'org-user-2', userId: 'user-2', courseId: 'course-1' }),
      deferred({
        organizationUserId: 'org-user-1',
        userId: 'user-1',
        courseId: 'course-2',
    ...
    const byMembership = new Map(notices.map((n) => [n.organizationUserId, n]));
    expect(byMembership.get('org-user-1')!.courses.map((c) => c.courseId)).toEqual([
      'course-1',
      'course-2',
    ]);
    expect(byMembership.get('org-user-2')!.courses.map((c) => c.courseId)).toEqual(['course-1']);
  });

  it('keeps one notice per membership when the same identity spans two organizations', () => {
    const notices = collectDeferredNotices([
      deferred({ organizationUserId: 'org-user-1', userId: 'user-1', courseId: 'course-1' }),
      deferred({ organizationUserId: 'org-user-9', userId: 'user-1', courseId: 'course-1' }),
    ]);
