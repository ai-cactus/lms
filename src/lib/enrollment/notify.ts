...
export function collectDeferredNotices(
  ...
  for (const item of deferred) {
    // Keyed by membership, not identity — one person can hold memberships in
    // several organizations, and each gets its own notice.
    let group = byUser.get(item.organizationUserId);
    if (!group) {
      group = {
        notice: {
      ...
      byUser.set(item.organizationUserId, group);
    }
...
export async function notifyCoursesAssigned(
  ...
  try {
    await createNotification({
      organizationUserId: notice.organizationUserId,
