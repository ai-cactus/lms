...
export function resolvePlaybackAuthz(
  ...
  const key = `authz:${organizationUserId}:${courseId}`;
 ...
 */
export function invalidatePlaybackAuthz(organizationUserId: string, courseId: string): void {
  authzCache.delete(`authz:${organizationUserId}:${courseId}`);
}
