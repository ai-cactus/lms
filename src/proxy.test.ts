 ...
 *
 * Focus: the critical gates this file had ZERO coverage for (no logged-in
 * cookie, role mismatch, MFA step-up, the worker onboarding gates), plus the
 * `/api/**` default-deny layer — because a middleware this security-critical
 * (every /dashboard, /onboarding, /worker and /api request) should not ship
 * changes with no regression guard at all.
 *
 * NOTE: this branch gates WORKER onboarding only. The admin-side onboarding
 * gate belongs to the multi-facility work and is deliberately not present.
 *
...
describe('proxy — onboarding gates', () => {
...
describe('proxy — Server Action requests bypass the onboarding gates', () => {
  ...
  it('applies the onboarding gate to an ordinary POST that is not a Server Action', async () => {
    ...
    const req = new NextRequest('http://localhost:3005/worker/courses/1', { method: 'POST' });
    req.cookies.set(WORKER_COOKIE, 'raw-token');
